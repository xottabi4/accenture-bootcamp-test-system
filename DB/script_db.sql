...
CREATE TABLE IF NOT EXISTS `onlinetestDB`.`user` (
  ...
  `security_code` VARCHAR(128) NULL,
  PRIMARY KEY (`user_id`))
ENGINE = InnoDB;
...
CREATE INDEX `testidfk123321_idx` ON `onlinetestDB`.`questions` (`test_id` ASC);
...
CREATE TABLE IF NOT EXISTS `onlinetestDB`.`question_option` (
  ...
  CONSTRAINT `qnofk1`
    ...
    REFERENCES `onlinetestDB`.`questions` (`question_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `qnofk1_idx` ON `onlinetestDB`.`question_option` (`question_no` ASC);
...
CREATE INDEX `qnofk2_idx` ON `onlinetestDB`.`answer_text` (`question_no` ASC);
...
CREATE INDEX `testidfk0001_idx` ON `onlinetestDB`.`user_response` (`test_id` ASC);
...
CREATE TABLE IF NOT EXISTS `onlinetestDB`.`user_test` (
  `user_test_id` INT(8) NULL AUTO_INCREMENT,
